...
![탐색 대시보드 — 떡상 영상·시계열·Outlier 분포·전체 데이터 탭](images/screenshot_dashboard.png)

탭 구성: 🔥 떡상 영상 · 📈 시계열 · 🎯 Outlier 분포 · 📊 전체 데이터  
필터: 기간·검색어·구독자 구간을 바꾸면 결과가 즉시 갱신된다. CSV 다운로드 지원.  
API 없이 동작 — 저장된 CSV만 읽는다 (`streamlit run app.py`).
