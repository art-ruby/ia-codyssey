...
**수집일 vs 게시 기간**: 수집은 2026-09-02 하루에 한 번 했다. 그날 검색한 영상들이
YouTube에 올라온 날짜가 2026-07-03 ~ 2026-09-01 범위에 분포한다. 검색 조건으로
최근 60일 이내 게시된 영상만 걸었기 때문이다(`SEARCH_DAYS = 60`). 수집 기간(며칠
동안 모았는가)이 아니라 **데이터의 시간 범위**를 뜻한다.
...
YouTube Data API v3는 Google이 Google Cloud Console 프로젝트 단위로 무료 제공하는
공개 API다. 기본 할당량이 있고, 2026년 6월부터 `search.list`가 별도 버킷으로
분리됐다. **버킷이 둘이다.**

| 버킷 | 한도 | 대상 메서드 | 정상 운영 소비 |
|---|---:|---|---|
| **search 버킷** | **100회/일** | `search.list` 만 | 16회 (16%) |
| **일반 버킷** | **10,000 units/일** | `videos.list` · `channels.list` · 댓글 등 나머지 전부 | ~1,630 units (16%) |

일반 버킷 상세:

| 작업 | units | 빈도 |
|---|---:|---|
| 수집 — `videos.list` + `channels.list` | 1,615 | 하루 1회 |
| 스냅샷 — `videos.list` | 12 | 하루 1회 |
| 채널 검증 | 40~57 | 가끔 |
| **합계** | **약 1,630** | |

두 버킷 모두 **태평양 자정(한국시간 오후 5시, 또는 DST 적용 시 오전 5시)**에
초기화된다. 한국 날짜 기준으로 세면 어긋난다 — `quota_day()` 는 이 점을 반영해 PT 기준으로 계산한다.

**개편 전후 차이**: 이전에는 `search.list` 1회가 일반 버킷에서 100 units를 소비했다.
지금은 search 버킷에서 1 unit을 쓰고, 일반 버킷과 완전히 독립이다. 일반 버킷
기준으로만 보면 검색 비용이 사라진 셈이라, 이제 일반 버킷은 사실상 비어 있다(0.2%).

**왜 16%만 쓰는가**: 어느 버킷이든 초과하면 그날 모든 API 호출이 막힌다. 스냅샷이
막히면 **그날 조회수를 영영 못 받는다** — 지나간 날은 다시 수집할 수 없어 시계열에
메울 수 없는 구멍이 생긴다. 그래서 `collector.py`에 같은 날 두 번째 실행을 막는
가드를 넣었다(`--force`로 우회). 정상 운영에서 16%를 유지해 개발 중 테스트·재실행
여유분과 검색어를 더 늘릴 여지를 남겨뒀다.

개발 중에는 수집을 하루 5번 돌려 일반 버킷 8,075 units(81%)까지 쓴 날이 있었다.
여섯 번이면 그날 스냅샷도 못 남기는 수준이었다.

**분석과 수집의 분리**: 수집은 하루 1회만 하고, 분석은 저장된 CSV만 읽도록 코드를
나눴다. API를 건드리지 않고 `python analysis.py`를 몇 번이든 돌릴 수 있는 이유다.
...
### ⚠️ 참여율이 바닥인 것도 잘라냈다 — 매크 방어

깃허브에서 유사 도구를 벤치마킹하다가(`docs/benchmark.md`) 발견한 방어선이다.
[yuben-app](https://github.com/shkuratovdesigner/yuben-app)은 「조회수 1,000회당
좋아요 1.5개 미만이면 매크(사서 본 조회수)로 본다」는 문턱으로 그런 영상을
순위에서 뺀다. **우리에게는 이 방어가 없었다.**

문턱값을 그대로 가져오지 않고 우리 데이터로 다시 확인했다.

```
참여율(좋아요÷조회수×1,000) 중앙 9.15 · 25% 5.05 · 75% 17.33  (623건 · 결측 9건)
1.5 미만  67건 (10.6%)
```

1.5는 우리 분포에서도 뚜렷한 하위 극단이라 그대로 썼다. `subscriber_view_ratio`
와 같은 방식으로 처리한다 — 행을 지우지 않고 **비율만 결측 처리**해서, 매크
의심 영상이 「구독자 대비 보너스」를 못 받게 한다. Video Score는 그 보너스가
빠지면 나이 보정 백분위만으로 계산되므로(`add_scores`), 영상이 사라지지도
점수가 깨지지도 않는다. `likes`가 없는 9건은 매크로 몰지 않았다 — «모른다»
와 «나쁘다»는 다르다.

**정직하게 말하면**, 구독자 1,000명 이상(비율을 재는 대상)만 놓고 보면 지금
이 순간 데이터에는 「참여율 바닥 & 구독대비 5배 이상」이 0건이다 — 즉 지금
이상치 목록이 매크로 오염돼 있었다는 증거는 아직 없다. 다만 위에서 보듯
롱테일 검색어로 구독자가 아주 적은 채널이 계속 늘고 있고, 그런 영상의
제목은 화면의 「검색어 관리」가 새 검색어 후보를 뽑는 재료이기도 하다.
걸리고 나서 고치면 그 사이 뽑은 후보가 이미 오염돼 있다. 그래서 증거가
나오기 전에 방어선부터 깔았다.
