...
def main():
    ...
    try:
        ...
        # collector 와 같은 방어. 예전에는 QuotaError 만 잡아서, 네트워크가
        # 한 번 끊기면 **쓴 양도 실패한 사실도 아무 데도 안 남았다.**
        #
        # 2026-09-03 08:57 자동 실행이 RemoteDisconnected 로 죽었는데
        # quota_log.csv 에 그날 줄이 아예 없었다. 화면은 «어제 것» 을 그대로
        # 보여줬고 사람은 스냅샷이 빠진 줄 몰랐다. 조용한 실패다(prd §10-④).
        client.log_quota("snapshot (실패)")
        print(f"\n스냅샷 도중 실패했습니다 — {type(e).__name__}: {e}")
        print(f"  쓴 양은 기록했습니다: {client.report()}")
        print("  이번 회차는 저장하지 않았습니다. 다시 돌리면 처음부터 받습니다.")
        return 1
