...
def _call_openai(prompt, key, model, base_url, timeout, images=None):
    """`images` 는 그림 주소 목록(선택). 안 넘기면 예전과 한 글자도 다르지 않다."""
...
def _call_gemini(prompt, key, model, base_url, timeout, images=None):
...
def _call_claude(prompt, key, model, base_url, timeout, images=None):
...
# 그림을 받을 수 있는 규격. openai 규격만 image_url 블록을 쓴다.
IMAGE_CAPABLE = {"openai"}
...
def images_supported():
    """지금 제공자가 그림을 받는가. 받지 못하면 호출부가 «못 봤다» 를 남겨야 한다."""
    return config.TRANSLATE_PROVIDER in IMAGE_CAPABLE


def ask_json(prompt, images=None):
    """프롬프트를 보내고 JSON 을 받는다. 실패하면 None 과 `last_error`.

    `images` 는 그림 주소 목록(선택). 그림 때문에 요청이 통째로 실패하면
    글자만으로 한 번 더 보낸다 — 썸네일 하나 때문에 분석 전체를 잃지 않는다.

    이 파일 이름은 «translate» 지만 제공자 교체 지점이 여기 하나뿐이라
    channel_fit·comment_analyzer 도 이 문을 쓴다. 호출 코드를 여러 곳에
    흩으면 제공자를 갈아탈 때 다 찾아다녀야 한다.
    """
    out = _extract_json(_ask(prompt, images=images))
    if out is None and images:
