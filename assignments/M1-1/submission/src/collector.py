...
def plan():
    ...
    2026-06-01 부터 search.list 는 자기 버킷을 쓴다 — 호출 1회가 1 unit 이고
    하루 100회다. 그래서 «units» 이 아니라 «횟수» 가 그대로 예산이다.
    """
    return len(seeds.terms()) * len(config.SEARCH_ORDERS) * config.MAX_PAGES_PER_QUERY
...
def main():
    ...
    print(f"  검색(search.list) {calls}/{budget}회 = {pct:.0f}%  "
          f"— 전용 버킷이라 videos.list 등과 예산을 나눠 쓰지 않습니다")
    ...
    # 한 번이 16~24회라 네댓 번이면 하루 100회가 사라진다.
    #
    # 예전에는 units 하나로 셌다. 그러면 스냅샷(하루 18 units)까지 같은
    # 예산에서 빼는 셈이라, 검색이 남았는데도 막는 일이 생겼다. 버킷이
    # 갈렸으니 세는 것도 갈랐다 — 여기서 보는 것은 **검색 횟수뿐**이다.
    searched = Client.searches_today()
    mine = Client.searches_today("collector")
    if mine and not args.force:
        print(f"\n오늘 이미 수집했습니다 (검색 {mine}회).")
        print("검색 결과는 몇 시간 만에 크게 바뀌지 않아 다시 돌릴 필요가 거의 없습니다.")
        print("\n  조회수만 새로 받기 (검색 0회 · 일반 버킷 약 12 units)")
        print("    python -m src.snapshot_collector")
        print("\n  그래도 다시 수집하기")
        print("    python -m src.collector --force")
        return 0
    if searched + calls > budget:
        print(f"\n오늘 이미 검색을 {searched}회 했습니다. {calls}회를 더 하면 "
              f"하루 상한 {budget}회를 넘어\n남은 하루 동안 검색을 못 합니다. "
              "내일 다시 돌리세요.")
        print("  (조회수 갱신은 검색을 쓰지 않으므로 지금도 됩니다: "
