...
> `python` 대화형 입력창(`>>>`)이 아니라 **프로젝트 경로가 표시되는 터미널**에서 실행.
> `>>>` 상태에서 `python src/main.py`를 입력하면 `SyntaxError`가 발생됐음.
...
프롬프트 추가 시 아래 6개 카테고리를 번호로 선택하거나, `0) 직접 입력`을 골라 새 카테고리를 입력 가능.
...
**기본 데이터로 4개** 등록되어 있어 과제 기준(최소 3개)을 충족함.
...
기본 과제에서는 프로그램을 실행하는 동안에만 프롬프트 데이터가 유지되고, 프로그램을 종료하면 추가하거나 변경한 내용이 사라집니다.

보너스 과제에서는 프로그램을 종료한 뒤에도 데이터가 남아 있도록 prompts.json 파일에 프롬프트 목록을 저장하고, 다음 실행 때 다시 불러오는 기능을 구현했습니다.

동작 방식
프로그램을 시작할 때 데이터 불러오기
프로그램을 실행하면 먼저 prompts.json 파일을 읽어 이전에 저장한 프롬프트 목록을 불러옵니다.
한글이 깨지지 않도록 UTF-8 형식을 사용합니다.
저장 파일이 없거나 손상된 경우
처음 실행하여 prompts.json 파일이 없거나, JSON 형식이 잘못되어 파일을 읽을 수 없는 경우에는 코드에 정의된 기본 프롬프트를 사용합니다.
이후 새로운 prompts.json 파일을 만들어 다시 저장합니다.
...
현재 프롬프트 전체 목록을 prompts.json에 저장합니다.

따라서 프로그램을 종료한 뒤 다시 실행해도 이전에 추가하거나 변경한 프롬프트가 그대로 유지됩니다.
...
프로그램 종료
    ↓
다음 실행 때 저장된 내용 다시 불러오기
JSON 저장 옵션
ensure_ascii=False
indent=2
ensure_ascii=False : 한글을 \uXXXX 형식으로 바꾸지 않고 그대로 저장합니다.
indent=2 : JSON 내용을 두 칸씩 들여쓰기하여 사람이 읽기 쉽게 저장합니다.
...
main.py는 src 폴더 안에 있고, prompts.json은 프로젝트 최상위 폴더에 있습니다.
...
코드에서는 다음 방식으로 프로젝트 최상위 폴더의 위치를 찾습니다.

Path(__file__).parent.parent
__file__ : 현재 실행 중인 main.py
첫 번째 .parent : src 폴더
두 번째 .parent : python-prompt-manager 프로젝트 폴더

따라서 어느 위치의 터미널에서 프로그램을 실행하더라도 항상 프로젝트 폴더에 있는 동일한 prompts.json 파일을 사용합니다.
...
실제로 입력한 명령을 **수행 순서 그대로** 정리,`기능 하나 구현 → 실행 확인 → 커밋` 순서를 지켰습니다.
